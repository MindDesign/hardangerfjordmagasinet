...
class Advanced_Ads_Tracking {
	...
	public function url_redirect() {
		...
		if ( $referrer && is_string( $referrer ) ) {
			...
			if ( $post ) {
				...
				$url = str_replace(
					array( '[POST_ID]', '[POST_SLUG]', '[CAT_SLUG]' ),
					array( $post->ID, $post->post_name, implode( ',', $cats_slugs ) ),
					$url
				);
			} else {
				/***
				...
				if ( 1 < count( $expl_url ) ) {
					...
					foreach ( $parsed as $key => $value ) {
						if ( ! in_array( $value, $p_holders, true ) ) {
	...
	public function add_tracking_link( $content, $ad ) {
		...
		if ( ( function_exists( 'advads_is_amp' ) && advads_is_amp() ) && $ad->type === 'plain' && ! $url ) {
			$amp_plain = true;
			$matches   = $this->get_url_from_string( $content );
			if ( ! empty( $matches[0] ) ) {
				$url = $matches[0];
				// Override placeholder dynamically.
				$placeholder = $url;
			}
		}
		...
		if ( $url ) {
			...
			if ( is_string( $link ) && $link !== '' ) {
				...
				if ( ! empty( $relationships ) ) {
					$rel_attribute = sprintf( ' rel="%s"', implode( ' ', $relationships ) );
				}

				$target = Advanced_Ads_Tracking_Util::get_target( $ad );

				// Add click tracking class.
				$class = ' class="adv-link"';

				// Wrap ad into tracking link if delivered by ad server or amp and url field is empty.
				if (
					(
						( function_exists( 'advads_is_amp' ) && advads_is_amp() && ! $amp_plain && ! $url )
						|| ( isset( $ad_options['placement_type'] ) && $ad_options['placement_type'] === 'server' )
					)
					&& $this->plugin->check_ad_tracking_enabled( $ad, 'click' )
				) {
					$content = '<a class="notrack" data-bid="' . $bid . '" href="' . $link . '"' . $rel_attribute . $target . '>' . $content . '</a>';
				} elseif ( strpos( $content, $placeholder ) !== false ) {
					...
					if ( $this->plugin->check_ad_tracking_enabled( $ad, 'click' ) ) {
						// Only use the tracking url if click tracking is enabled.
						$content = str_replace( $placeholder, $link, $content );
						// Add notrack-class to disable js-based tracking for this link.
						$content = str_replace( 'href="', 'class="notrack" data-bid="' . $bid . '" href="', $content );
					} else {
						$content = str_replace( $placeholder, esc_url( $url ), $content );
					}
				} elseif ( ( ( function_exists( 'advads_is_amp' ) && advads_is_amp() ) || $ad->options( 'tracking.cloaking' ) ) && $url ) {
					// Url field is set on amp or link cloaking is activated.
					$content = '<a class="notrack" data-bid="' . $bid . '" href="' . $link . '"' . $rel_attribute . $target . '>' . $content . '</a>';
				} else {
					// Wrap ad into original link.
					$content = '<a href="' . esc_url( $url ) . '"' . $rel_attribute . $target . $class . '>' . $content . '</a>';
				}
			}
		}
