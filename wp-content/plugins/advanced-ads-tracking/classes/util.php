...
final class Advanced_Ads_Tracking_Util {
	 ...
	 * @var array
	 */
	protected $bots = array( 'AspiegelBot', 'BingPreview', 'bingbot', 'datanyze', 'ecosia', 'Googlebot', 'Google-AMPHTML', 'GoogleAdSenseInfeed', 'Hexometer', 'mediapartners', '^Mozilla\/5\.0$' );
	...
	public function collect_blog_data() {
		...
		if ( ! isset( $this->blog_data['ajaxurls'][ $bid ] ) ) {
			...
			if ( $this->plugin->is_legacy_ajax() || ! file_exists( WP_CONTENT_DIR . '/ajax-handler.php' ) ) {
	 ...
	 */
	public static function get_target( $ad ) {
		$ad_options      = $ad->options();
