...
class AdvanvedAdsNetworkGam extends AdvancedAdsAdNetwork {
	 ...
	 */
	getSelectedId() {
		console.log('AdvanvedAdsNetworkGam >> getSelectedId');
	}
	...
	onDomReady() {
		...
		jQuery(document).on('click', '#advads-gam-table .dashicons-update,#advads-gam-table-head .dashicons-update,.gam-update-icon', function () {
			if (('undefined' !== gamAdvancedAdsJS && 'no' == gamAdvancedAdsJS.hasGamLicense) || $(this).hasClass('disabled')) {
				return;
			}
			that.refershAdsList();
		});
...
AdvancedAdsGamConnect.prototype = {
	...
	confirmCodeResponse: function (response, status, XHR) {
		var that = this;
