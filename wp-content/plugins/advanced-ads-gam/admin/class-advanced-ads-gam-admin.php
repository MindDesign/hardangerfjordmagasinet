...
class Advanced_Ads_Gam_Admin {
	...
	public function save_ad( $post_id ) {
		...
		if ( ! current_user_can( Advanced_Ads_Plugin::user_cap( 'advanced_ads_edit_ads' ) ) // only use for ads, no other post type.
			 ...
			 || Advanced_Ads::POST_TYPE_SLUG != $_post['post_type']
		...
		if ( isset( $_post['advanced_ad']['gam'] ) && isset( $_post['advanced_ad']['gam']['key'] ) ) {
			$keyval = array();
			foreach ( $_post['advanced_ad']['gam']['key'] as $key => $value ) {
				if ( isset( $_post['advanced_ad']['gam']['value'] ) && is_array( $_post['advanced_ad']['gam']['value'] ) ) {
					$keyval[] = array(
						'type'       => $_post['advanced_ad']['gam']['type'][ $key ],
						'key'        => $value,
						'value'      => $_post['advanced_ad']['gam']['value'][ $key ],
						'onarchives' => $_post['advanced_ad']['gam']['onarchives'][ $key ],
					);
				}
			}
	...
	public function get_key_values_types() {
		...
		$kvs['postmeta'] = array(
			...
			'html' => '<input type="text" id="advads-gam-kv-value-input" /><span class="description">' . esc_html__( 'Enter the post meta name as a value.', 'advanced-ads-gam' ) . '</span>',
		);

		$kvs['usermeta'] = array(
			'name' => esc_html__( 'User meta', 'advanced-ads-gam' ),
			'html' => '<input type="text" id="advads-gam-kv-value-input" /><span class="description">' . esc_html__( 'Enter the user meta name as a value.', 'advanced-ads-gam' ) . '</span>',
		);
	...
	public function get_api_key() {
		...
		$_post = wp_unslash( $_POST );
		if ( false !== wp_verify_nonce( $_post['nonce'], 'gam-connect' ) ) {
	...
	private function soap_get_all_networks( $token ) {
		...
		$soap_var = new SoapVar( $req_h, SOAP_ENC_OBJECT, 'RequestHeader', $ns );
			...
			if ( empty( $net_array ) ) {
				...
				if ( isset( $_post['token_data'] ) ) {
					...
					if ( isset( $net->rval ) && isset( $net->rval->networkCode ) ) {
						...
						$this->save_tokens( $_post['token_data'] );
	...
	public function ajax_get_all_networks() {
		...
		if ( false !== wp_verify_nonce( $_post['nonce'], 'gam-connect' ) ) {
			...
			if ( isset( $_post['token_data'] ) ) {
				$token = $_post['token_data']['access_token'];
			} else {
			...
			if ( self::has_soap() ) {
				...
				$result = $this->nosoap_get_all_networks( $_post['token_data'] );
				wp_send_json( $result );
			}
		}
		die;
	...
	public function test_if_api_enabled() {
		...
		if ( false !== wp_verify_nonce( $_post['nonce'], 'gam-selector' ) || false !== wp_verify_nonce( $_post['nonce'], 'gam-connect' ) ) {
	...
	public function ajax_revoke_tokken() {
		...
		if ( false !== wp_verify_nonce( $nonce, 'gam-connect' ) ) {
			$gam_option = Advanced_Ads_Network_Gam::get_option();
	 ...
	 */
	public function get_all_gam_ads() {
		return Advanced_Ads::get_ads(
			array(
