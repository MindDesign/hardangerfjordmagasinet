...
class Advanced_Ads_Pro_Module_Cache_Busting {
    ...
    protected function get_override_content( $query ) {
        ...
        if ( ! empty( $query['method'] ) && 'placement' === $query['method'] && ! empty( $query['id'] )  ) {
            ...
            $content .= $this->create_wrapper( $query['elementid'], $placement_id );
        }
     ...
     * @return array $args.
     */
    private function create_wrapper( $element_id, $placement_id = '' ) {
        $class = $element_id;
        if ( $placement_id ) {
        ...
        return '<div class="' . $class . '" id="' . $element_id . '"></div>';
    }
