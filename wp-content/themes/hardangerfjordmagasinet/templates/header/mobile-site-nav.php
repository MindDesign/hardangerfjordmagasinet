    ...
    <nav id="site-navigation" class="" role="navigation" aria-label="<?php esc_attr_e( 'Primary menu', 'hardangerfjordmagasinet' ); ?>">
        ...
        <?php if ( !empty ( $items ) ) : ?>
            <?php foreach ( $items as $item ) : ?>
                ...
                <a href="<?php echo $item->url; ?>" class="py-4 block w-full text-center lg:py-auto lg:inline-block lg:w-auto lg:text-left lg:pl-4 text-xl text-white uppercase <?php echo $current; ?>"><?php echo $item->title; ?></a>
            <?php endforeach; ?>
        <?php endif; ?>
    </nav><!-- #site-navigation -->
<?php endif; ?>
